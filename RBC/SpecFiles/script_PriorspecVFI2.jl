...
function fPriorDraw_VFI2()


    # Draw model parameter objects:

    ρz     = rand(Beta(fMyBeta(0.6,0.15)...))   #rand()
    σz     = rand(InverseGamma(fMyInvGamma(4.0,0.8)...))/100

    ρb     = rand(Beta(fMyBeta(0.6,0.15)...))   #rand()
    σb     = rand(InverseGamma(fMyInvGamma(4.0,0.8)...))/100
    ...
    ϕ2     = rand(Normal(0,75)) #note: 2nd argument is StD, not Var!
...
function fPriorLogEval_VFI2(θ)
    ...
    vP[2]  = pdf(InverseGamma(fMyInvGamma(4.0,0.8)...),σz*100)

    vP[3]  = pdf(Beta(fMyBeta(0.6,0.15)...),ρb)
    vP[4]  = pdf(InverseGamma(fMyInvGamma(4.0,0.8)...),σb*100)
