...
# Script to calibrate number of BSPF particles for LL-eval. under mL3
...
useDataModelspec    = "L1"      # use "L3" to calibrate BSPF, use "L1" to check whether in this case LL under L3 is lower than under L1
...
@everywhere include(sMyPath * "SpecFiles/" * string("script_ModelspecL3.jl") )
...
# Mbspf = 500
# loglik, timeSolution, timePF = fLL_L3(θ0,Mbspf)
...
@sync @distributed for mm = 1:M
    println(mm)
    vDraws[mm], mTimes[mm,1], mTimes[mm,2] = fLL_L3(θ0,Mbspf)
end
...
timeSol     = vTimes[1]
timeBSPF    = vTimes[2]
ppp         = histogram(vDraws,fillalpha=0.5,label="mL3")
...
sFileForOutput         = string("plot_mL3_BSPFcheck_M",Mbspf,"_dgp",DGPspec,"_m",useDataModelspec,".png")
