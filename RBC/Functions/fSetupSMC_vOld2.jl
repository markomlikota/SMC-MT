...
function fSMC(tSettings,sOutputPath,sOutputFilePrefix,writeFiles=1,setSeed=0,mInitParticles=zeros(10,5),vPIndDraw=zeros(5),mContinue_StageAll_Stats=0,mContinue_WLLP=0,mContinue_Particles=0)
    ...
    # - mContinue_... matrices are output from an interrupted SMC run. When supplied, SMC continues from where it left off.
    ...
    if continueInterruptedRun
        ...
        if writeFiles > 0
            ...
            # CSV.write(sOutputPath * savename,  DataFrame(vmParticles[1]), append=true)
            ...
            # CSV.write(sOutputPath * savename,  DataFrame([vvWeights[1] vmLogLiks[1] vvPriors[1]]), append=true)
            ...
            if writeFiles == 2
                ...
                # CSV.write(sOutputPath * savename,  DataFrame(permutedims(["phi","c","accRate","logMDD","time","ESS"])), header=false)
                # CSV.write(sOutputPath * savename,  DataFrame([vϕ[1] vc[1] vAcceptRate[1] vLogMDD[1] vTimes[1] vESS[1]]), append=true)
    ...
    for iiPhi = iiPhiFirst : Nϕ
        ...
        if writeFiles == 2
            ...
            # CSV.write(sOutputPath * "IntermediateStages/" * savename,  DataFrame(permutedims(vParLabs)), header=false)
            # CSV.write(sOutputPath * "IntermediateStages/" * savename,  DataFrame(vmParticles[nPhi]), append=true)
            fMyCSVWRITE(sOutputPath * savename,vmParticles[nPhi],vParLabs)

            savename            =  sOutputFilePrefix * string("Stage",nPhi,"_WLLP.csv")
            # CSV.write(sOutputPath * "IntermediateStages/" * savename,  DataFrame(permutedims(["weight","ll","llt","prior"])), header=false)
            # CSV.write(sOutputPath * "IntermediateStages/" * savename,  DataFrame([vvWeights[end] vmLogLiks[end] vvPriors[end]]), append=true)
            fMyCSVWRITE(sOutputPath * savename,[vvWeights[end] vmLogLiks[end] vvPriors[end]],["weight","ll","llt","prior"])

            savename            =  sOutputFilePrefix * "StageAll_Stats.csv"
            # CSV.write(sOutputPath * savename,  DataFrame([vϕ[nPhi] vc[nPhi] vAcceptRate[nPhi] vLogMDD[nPhi] vTimes[nPhi] vESS[nPhi]]), append=true)
    ...
    if writeFiles > 0
        ...
        # CSV.write(sOutputPath * savename,  DataFrame(permutedims(vParLabs)), header=false)
        # CSV.write(sOutputPath * savename,  DataFrame(vmParticles[end]), append=true)
        fMyCSVWRITE(sOutputPath * savename,vmParticles[end],vParLabs)

        savename            =  sOutputFilePrefix * "StageLast_WLLP.csv"
        # CSV.write(sOutputPath * savename,  DataFrame(permutedims(["weight","ll","llt","prior"])), header=false)
        # CSV.write(sOutputPath * savename,  DataFrame([vvWeights[end] vmLogLiks[end] vvPriors[end]]), append=true)
        fMyCSVWRITE(sOutputPath * savename,[vvWeights[end] vmLogLiks[end] vvPriors[end]],["weight","ll","llt","prior"])
        ...
        if writeFiles != 2
            savename            =  sOutputFilePrefix * "StageAll_Stats.csv"
            # CSV.write(sOutputPath * savename,  DataFrame(permutedims(["phi","c","accRate","logMDD","time","ESS"])), header=false)
            # CSV.write(sOutputPath * savename,  DataFrame([vϕ vc vAcceptRate vLogMDD vTimes vESS]), append=true)
        ...
        # CSV.write(sOutputPath * savename,  DataFrame([Nϕ N timeTotal sum(vLogMDD)]), append=true)
