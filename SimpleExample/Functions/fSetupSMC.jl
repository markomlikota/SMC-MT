...
function fGetParticleStats(vWeights,mParticles,vQuantiles=[0.05,0.95])
    ...
    if nP > 1
        ...
        vVars           = var(mParticles,vProbWeights)
    end
    vStds           = sqrt.(vVars)
    mCI             = zeros(nP,nQ)
...
function fSMC(tSettings,sOutputPath,sOutputFilePrefix,setSeed=0,mInitParticles=zeros(10,5),vPIndDraw=zeros(5),writeFiles=1)
    ...
    # else
    #     vmParticles       = Vector{Vector{Float64}}(undef,1)
    # end
    ...
    if writeFiles == 1
        ...
        CSV.write(sOutputPath * savename,  DataFrame([vϕ vc vAcceptRate vLogMDD]), append=true)
