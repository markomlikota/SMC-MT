...
function fMyInvGamma(s,v)

    # Goes from specification in HS(15),AS(07) to that of Julia & Wikipedia.
    # Note: need to divide draw by 100.

    # Note that the above two references write (s,v) in their tables, but (v,s) in the pdf expression in the text below the table... in any case, v is degrees of freedom, so e.g. use (s,v) = (0.4,4.0)
