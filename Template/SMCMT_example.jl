...
# This script shows "SMCMT.jl" at work with a very simple example: based on two series (N=2) and T=3 (so 6 data points), 
# we estimate the means of the two series (μ1,μ2) using a likelihood and priors consisting of independent Normals with fixed variances. 
# The prior for μ2 is truncated at μ2 > -2. M0 estimates only μ1, M1 estimates both.
...
sTemperingType = "MT"          ### "LT" or "MT": 
                            #   LT : likelihood-tempering estimation of M0
                            #   MT : model-tempering estimation of M1 (based on preliminary LT-estimation of M0)

ϕ_Nϕ_M0         = 0.5          ### scalar ∈ (0,1], indicating whether LT-estimation of M0 is stopped early (ψ_* in paper;
...
sOutputPathBothModels             =  sMyPath * "Output/"               ### set path where output shall be stored, ending with "/" ; could use "Output" folder in overall path.
...
mData                       = readdlm(sMyPath * "Data/Data.csv", ',', Float64, '\n'; skipstart=1)          ### load data (e.g. (T x N) matrix); could use folder "Data"
...
@everywhere vParLabs        = ["mu1","mu2"]          ### define nP x 1 vector of strings (parameter labels), corresponding to scalar-parameters in nP x 1 vector θ
...
@everywhere begin                       # each worker needs to know these functions
    ...
    function fLL_M1(θ)                      # log-likelihood function for model M1 (model of interest)
        ...
        LL      = sum([logpdf(Normal(μ1,1),mData[tt,1]) for tt=1:T]) + sum([logpdf(Normal(μ2,1),mData[tt,2]) for tt=1:T])
    ...
    function fPriorLogEval_M1(θ)            # evaluates prior at θ under model M1
        ...
        vP[1]  = logpdf(Normal(0,3),μ1)
        vP[2]  = logpdf(Normal(0,3),μ2)
...
@everywhere begin
    ...
    function fLL_M0(θ)                      # log-likelihood function for model M0 (approximating model)
        ...
        LL      = sum([logpdf(Normal(μ1,2),mData[tt,1]) for tt=1:T]) 
...
if sTemperingType == "LT"
    ...
    setSeed             = 547              ### set seed for SMC estimation
