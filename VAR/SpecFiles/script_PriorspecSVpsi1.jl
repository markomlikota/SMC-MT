...
S_pr_SVpsi     = deepcopy(S_pr)
v_pr_SVpsi     = deepcopy(v_pr)

μ_pr_SVpsi     = deepcopy(μ_pr)
P_pr_SVpsi     = deepcopy(P_pr)



# Define fPriorDraw_Mi(), fPriorLogEval_Mi(ϑMi), fIsDrawValid_Mi(ϑMi):

function fPriorDraw_SVpsi()
    ...
    Σ, Φ        = fOneDrawMNIW(μ_pr_SVpsi,P_pr_SVpsi,S_pr_SVpsi,v_pr_SVpsi)
    Φε          = cholesky(Σ).L
    ...
    ϑ           = fVARObjectsToϑ_SVpsi(Φ, Φε, vρ, vξ)
...
function fPriorLogEval_SVpsi(ϑ)
    ...
    prprΦΣ        = fEvalMNIW(Φ,Σ,μ_pr_SVpsi,P_pr_SVpsi,S_pr_SVpsi,v_pr_SVpsi)
    ...
    vP[mParaPointers_SVpsi[1,1]]                       = prprΦΣ
    vP[mParaPointers_SVpsi[3,1]:mParaPointers_SVpsi[3,2]] = vprprρ
    vP[mParaPointers_SVpsi[4,1]:mParaPointers_SVpsi[4,2]] = vprprξ
...
function fIsDrawValid_SVpsi(ϑ)
    ...
    # vValid[mParaPointers_SVpsi[1,1]]                     = isVARstationary
    vValid[mParaPointers_SVpsi[3,1]:mParaPointers_SVpsi[3,2]] = vIsSVstationary
    vValid[mParaPointers_SVpsi[4,1]:mParaPointers_SVpsi[4,2]] = vAreXisPositive
