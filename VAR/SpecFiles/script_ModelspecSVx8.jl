...
vParLabs_SVx8         = [vec(mLabs_Φ)..., mLabs_Σ[tril(trues(size(mLabs_Σ)))]..., vLabs_vρ..., vLabs_vξ...]


vGroupLengths_SVx8    = [n*k, n*(n+1)/2, n, n]
mParaPointers_SVx8    = fGetParaPointers(vGroupLengths_SVx8)


# Functions to go from ϑ to VAR objects and back:

function fϑtoVARObjects_SVx8(ϑ)

    n     = 2
    p     = 1
    k     = n*p + 1

    Φ     = reshape(ϑ[mParaPointers_SVx8[1,1]:mParaPointers_SVx8[1,2]],k,n)

    Σ                       = zeros(n,n)
    Σ[tril(trues(size(Σ)))] = ϑ[mParaPointers_SVx8[2,1]:mParaPointers_SVx8[2,2]]
    mIndHelp                = tril(trues(size(Σ)),-1)
    Σ[mIndHelp']            = Σ[mIndHelp]

    vρ    = ϑ[mParaPointers_SVx8[3,1]:mParaPointers_SVx8[3,2]]

    vξ    = ϑ[mParaPointers_SVx8[4,1]:mParaPointers_SVx8[4,2]]
...
function fVARObjectsToϑ_SVx8(Φ, Σ, vρ, vξ)
